    ...
    *data* keys:
    ...
    if grid_context:
        ...
        system += (
            ...
            '- run_what_if: {"type":"run_what_if","optimization_id":"<optional, uses current>","uptake_override":<optional float 0-1>,"cost_override":<optional float>,"profile_id":"<optional>"}\n'
            '  Re-runs the Monte Carlo simulation with overridden assumptions. Use when user asks "what if uptake is 20%?" or "what if cost drops to $50?".\n'
            '  uptake_override: set all incentive uptake rates to this value (e.g. 0.2 for 20%).\n'
            '  cost_override: set all incentive costs to this value in dollars.\n'
            '  profile_id: optional, run what-if for one profile only.\n'
